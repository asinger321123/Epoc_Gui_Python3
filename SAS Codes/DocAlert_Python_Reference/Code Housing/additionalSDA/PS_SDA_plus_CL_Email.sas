...
%do i=1 %to 1;
	%if &sda_occ. ne "" %then %do;
		%if &sda_spec. ne "" %then %do;
			...
			select distinct userid, occupation_key, specialty_key, state_key
...
%do i=1 %to 1;
	...
	%if &applyToSda. = Yes %then %do;
		%if &queryByState. = Yes %then %do;
			proc sql;
			create table sda_dedupe_2/*inc*/ as
			Select a.*, b.state_key FROM new_bb/*inc*/ a
			inner join bb/*inc*/ b
		...
		%if &queryByZip. = Yes %then %do;
			...
			proc sql;
			create table sda_dedupe_2/*inc*/ as
			Select a.*, b.postal_code FROM new_bb/*inc*/ a
			inner join bb/*inc*/ b
			on a.userid=b.userid
...
%global lm_sda_count/*inc*/;
%global lm_sda_price/*inc*/;
	...
	%if &list_match_type. ne None or &matched_file. ne "" %then %do;
		...
		proc sql;
		create table lm_plus_sda1/*inc*/ as
		select priceTotal format=13. FROM list_sda_total;
		run;

		data lm_plus_sda/*inc*/;
		set lm_plus_sda1/*inc*/;
		run;

		proc sql noprint;
		select priceTotal format=dollar10. into: lm_sda_price/*inc*/ from lm_plus_sda/*inc*/;
		quit;
		%put &lm_sda_price/*inc*/;

		/*CREATE BDA LIST MATCH TOTAL VARIABLE*/
		proc sql;
		create table lm_sda_users1/*inc*/ as
		select listTotal as count1 from list_sda_total;
		run;

		data lm_sda_users/*inc*/;
		set lm_sda_users1/*inc*/;
		run;

		proc sql noprint;
		select listTotal format=comma8. into: lm_sda_count/*inc*/ from list_sda_total;
		quit;
		%put &lm_sda_count/*inc*/;		
	%end;
...
/*					union*/
		...
		proc sql noprint;
		select sum(count1) format=comma8. into: lm_sda_count/*inc*/ from lm_sda_users/*inc*/;
		quit;
		%put &lm_sda_count/*inc*/;
...
/*					union all*/
		...
		proc sql noprint;
		select sum(amount1) format=dollar10. into: lm_sda_price/*inc*/ from lm_plus_sda/*inc*/;
		quit;
		%put &lm_sda_price/*inc*/;		
	%end;
...
%macro sda_table_title;
	%if &list_match_type. ne None or &matched_file. ne "" %then %do;
		...
		^{style [FONT=(Calibri,11Pt) color=#1f4e79 ] The total size of the client list and Specialty is } ^{style [Font_weight=bold FONT=(Calibri,11Pt) color=#1f4e79] &lm_sda_count/*inc*/ } ^{style [FONT=(Calibri,11Pt) color=#1f4e79] campaign eligible users. The total price to target all these users is} ^{style [Font_weight=bold FONT=(Calibri,11Pt) color=#1f4e79] &lm_sda_price/*inc*/ } ^{style [FONT=(Calibri,11Pt) color=#1f4e79] per wave.}
		"
		;
	%end;
	...
	%if &list_match_type. = None and &matched_file. = "" %then %do;
		...
		title "
		^{style [Font_weight=bold textdecoration=underline FONT=(Calibri,11Pt) color=#1f4e79 ] Requested Add-on Pricing: } ^{newline 2}
		^{style [FONT=(Calibri,11Pt) color=#1f4e79 ] The total size of the Specialty is } ^{style [Font_weight=bold FONT=(Calibri,11Pt) color=#1f4e79] &lm_sda_count/*inc*/ } ^{style [FONT=(Calibri,11Pt) color=#1f4e79] campaign eligible users. The total price to target all these users is} ^{style [Font_weight=bold FONT=(Calibri,11Pt) color=#1f4e79] &lm_sda_price/*inc*/ } ^{style [FONT=(Calibri,11Pt) color=#1f4e79] per wave.}
		"
		;
	%end;
