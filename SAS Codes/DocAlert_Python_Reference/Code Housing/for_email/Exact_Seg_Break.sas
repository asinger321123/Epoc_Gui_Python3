************************************************************************************************
* CODE NAME: PS_NPI_ME_Exact_Segmentation                                                      *
* AUTHOR: Adam Rubin                                                                           *
* CREATION DATE: 20170109                                                                      *
* MACRO VERSION: 1.0                                                                           *
* PURPOSE: Price Client List by NPI or ME with segmentation                                    *
*                                                                                              *
* REVISIONS:                                                                                   *
*  VERSION        DATE       AUTHOR              DESCRIPTION                                   *
*  -------      ---------  	--------        ----------------------                             *
*   3.0	        20170301   Adam Rubin       Updated Rate Card Sheet                            *
************************************************************************************************;
...
Proc sql;
Create table lmb as 
Select me_temp2 as me, npi, &seg., &mainCols.
from lmb_append;
quit;
...
proc sql;
  ...
  select w.*, cu.userid, cu.user_account_key, cu.primary_account_ind, cu.occupation_key, cu.specialty_key, cu.state_key, cu.last_session_dt, cu.country_key
	from lm1 w
  left join da.customer_user_nppes cu on w.npi = cu.npi;
quit;
...
proc sql;
  ...
  select w.*, cu.userid, cu.user_account_key, cu.primary_account_ind, cu.occupation_key, cu.specialty_key, cu.state_key, cu.verified_me_num, cu.last_session_dt, cu.country_key
	from has_me_num w
  left join da.customer_user_nppes cu on w.me = cu.verified_me_num
;quit;
...
/*inner join da.customer_user_nppes (keep = userid user_account_key last_session_dt) b on a.userid=b.userid*/
...
%do i=1 %to 1;
	%if &applyToClientList. = Yes %then %do;
		%if &queryByState. = Yes %then %do;
			...
			inner join da.customer_user_nppes b
...
select count(userid) into :registered_count from registered_users;
...
proc freq data=lmt._matched_%trim(&ncount.); TITLE1 "Client List Occupation Breakout"; tables occupation_key occupation_key / out=occ; run; TITLE1;
...
tables segment*flag / norow nocol nopercent;
run;
...
select &pricing, occspec, count(userid) as ncount 
from usr_profile4
group by 1,2;
quit;
