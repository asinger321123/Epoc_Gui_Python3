...
select distinct a.*, b.occupation_key, b.specialty_key, b.last_Session_dt
from temp_users1 a 
...
select distinct a.*, cats(epoc_fname,epoc_lname,epoc_zip) as combo, b.last_session_dt
from registered_users_1 a
inner join temp_customer_users_nppes (keep = userid user_account_key last_session_dt) b on a.userid=b.userid
where b.last_session_dt GE &user_lookup_date.-180
and b.last_session_dt LE &user_lookup_date.
group by a.epoc_me, a.epoc_npi, combo
having (count(a.epoc_me) > 1 or count(a.epoc_npi) > 1) and count(combo) > 1 and b.last_Session_dt = max(b.last_Session_dt)
...
/*proc sort data=latestDupUsers out=singleUsers dupout=mydups nodupkey;*/
/*by epoc_me epoc_npi combo &seg_nocommas.;*/
/*run;*/
...
%macro merck_organic;
%do i=1 %to 1;
	%if &merck_organic_list. = Yes %then %do;
		proc sql;
		create table merck_organic as
		select a.customer_id as target_customer, b.customer_id as final_customer_id, b.userid from target a
		left join final b on a.customer_id=b.customer_id;
		quit;

		proc sql;
		alter table merck_organic
		add membership_match numeric, vendor char(50);
		quit;

		proc sql;
		update merck_organic
		set membership_match=0
		where final_customer_id is null;
		quit;

		proc sql;
		update merck_organic
		set membership_match=1
		where final_customer_id is not null;
		quit;

		proc sql;
		update merck_organic
		set vendor='EPOCRATES';
		quit;

		proc sql;
		create table merck_final as
		select membership_match, target_customer as customer_id, vendor
		FROM merck_organic;
		quit;

		proc sql;
		create table epoc_final as
		select membership_match, target_customer as customer_id, vendor, userid
		FROM merck_organic
		where membership_match = 1;
		quit;

		proc sql;
		create table merck_doublecheck as
		select * from merck_final
		where membership_match = 1;
		quit;

		proc export data=merck_final outfile="&filepath.\MATCH_LIST_EPOCRATES_YYYY_MM_DD.csv" dbms=csv replace; run;
		proc export data=epoc_final outfile="&filepath.\MATCH_LIST_EPOCRATES_YYYY_MM_DD_USERIDS.csv" dbms=csv replace; run;
	%end;
%end;
%mend;
%merck_organic;
...
/*proc export data=lmt._matched_%trim(&ncount.) outfile="&filepath.\supp_final.csv" dbms=csv replace; run;*/
...
/*Start of new code for guarantee Rates*/
proc import datafile="P:\Epocrates Analytics\List Match\Guarentee Rates\Sample Rates for Rate Card.csv"
     out=guarantee_rate
     dbms=csv
	 replace;
	 GUESSINGROWS=10000;
...
proc sql;
create table usr_profile_for_guarantee as
select 
distinct userid,
case when (f1.label = 'MD' OR f1.label = 'DO') then CATX(' - ','MD/DO',f2.label) 
	 when (f1.label = 'NP' OR f1.label = 'PA') then CATX(' - ','NP/PA',f2.label) 
	 else f1.label end as occspec
/*from lmt._matched_%trim(&ncount.) t*/
from lmt._matched_%trim(&ncount.) t
left join edwf.format_lookup (where=(fmtname='OK2ON')) f1 on t.occupation_key = f1.key_value
inner join edwf.format_lookup (where=(fmtname='SK2S2X')) f2 on t.specialty_key = f2.key_value;
...
proc sql;
create table guarantee_users as
select distinct(a.occspec) as usr_occSpec, b.Occ_Spec as rate_occSpec, count(a.userid) as users, b.rate
from usr_profile_for_guarantee a
left join guarantee_rate b on a.occspec = b.Occ_Spec
group by 1, 4;
quit;
...
/*select disticnt occ_spec from usr_profile;*/
/*quit;*/


data guarantee_users_rate;
set guarantee_users;
guar_openers = users*rate;
run;


%global final_guar_rate;
%global cl_users_total;
%global cl_guarantee_total_rounded;

proc sql;
select sum(users) into :cl_users_total from guarantee_users;
select floor(sum(guar_openers)) into :cl_guarantee_total_rounded from guarantee_users_rate;
quit;

data cl_guarentee_rate_final;
guar_opens = &cl_guarantee_total_rounded.;
guar_opens_rate = &cl_guarantee_total_rounded./&cl_users_total.;
guar_opens_rate_final = guar_opens_rate;
format guar_opens_rate_final percent8.2;
run;

proc sql;
select guar_opens_rate into :final_guar_rate from cl_guarentee_rate_final;
quit;
...
%do i=1 %to %sysfunc(countw(&seg_nocommas));
   ...
   %if &run_30_60_90. = Yes %then %do;
		...
		proc sql;
		create table _90_active_&_30_60_90_Segs. as
		select distinct a.&_30_60_90_Segs, count(distinct a.userid) as _90_Days_Active
		from lmt._matched_%trim(&ncount.) a
		inner join temp_customer_users_nppes b on a.userid=b.userid
		where b.last_session_dt GE today()-91
		GROUP BY a.&_30_60_90_Segs;

		create table _60_active_&_30_60_90_Segs. as
		select distinct a.&_30_60_90_Segs, count(distinct a.userid) as _60_Days_Active
		from lmt._matched_%trim(&ncount.) a
		inner join temp_customer_users_nppes b on a.userid=b.userid
		where b.last_session_dt GE today()-61
		GROUP BY a.&_30_60_90_Segs;

		create table _30_active_&_30_60_90_Segs. as
		select distinct a.&_30_60_90_Segs, count(distinct a.userid) as _30_Days_Active
		from lmt._matched_%trim(&ncount.) a
		inner join temp_customer_users_nppes b on a.userid=b.userid
		where b.last_session_dt GE today()-31
		GROUP BY a.&_30_60_90_Segs;
		quit;

/*		data _90_active_&_30_60_90_Segs.;*/
...
/*		run;*/
		...
		proc print data=_90_active_&_30_60_90_Segs. noobs;
		run;

		proc print data=_60_active_&_30_60_90_Segs. noobs;
		run;

		proc print data=_30_active_&_30_60_90_Segs. noobs;
		run;
