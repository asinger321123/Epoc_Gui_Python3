...
/* three options: 1)pipe--columns are separated by '|' ; 
	...
	%let file_type = tab;
	%let keepvars= fname lname npi zip me, &mainCols.; 
...
Proc sql;
Create table lmb as 
Select me_temp2 as me, lname, fname, zip, npi, &mainCols. 
from lmb_append;
quit;
...
select me, lname, fname, zip, &mainCols.
from lm2
union
select me, lname, fname, zip, &mainCols.
...
select lname, fname, zip, &mainCols.
from no_me_num
union
select lname, fname, zip, &mainCols.
from not_CE_me_user
...
PROC SQL;
  ...
  SELECT distinct userid, occupation_key, specialty_key, &mainCols.
  FROM &lmtresults 
...
Proc sql;
create table schedule_userids as
select a.*, b.userid
from UAW.SPONSORED_DOCALERT_TDR_PROD a
inner join temp_customer_users_nppes b on a.user_account_key=b.user_account_key
WHERE a.docalert_schedule_key -937 in (623265,
623263,
623256,
623258,
623259,
...
/*Proc sql;*/
/*create table distinct_schedule_ids as*/
...
proc sql;
alter table final
add user_type char(50);
quit;

proc sql;
create table interaction_final_1 as
select * from final a
left join schedule_userids b 
on a.userid = b.userid
where b.userid is not null;
quit;

proc sql;
update interaction_final_1
set user_type = 'engager'
WHERE viewed = 1;
quit;

proc sql;
update interaction_final_1
set user_type = 'non_engager'
WHERE viewed = 0;
quit;

proc sql;
create table interaction_final as
select userid, occupation_key, specialty_key, user_type
FROM interaction_final_1;
quit;

proc sql;
create table unique_engagers as
select distinct * from interaction_final
where user_type = 'engager';
quit;

proc sql;
create table unique_non_engagers as
select distinct * from interaction_final
where user_type = 'non_engager';
quit;

proc sql;
create table merged_users as
select * from unique_engagers
union
select * from unique_non_engagers
where userid not in (select userid from unique_engagers);
quit;


proc sql noprint;
select count(userid) into :ncount from merged_users;
quit;

proc sort data = merged_users out=lmt._matched_%trim(&ncount.) nodupkey; by userid occupation_key specialty_key; run;
...
select distinct userid, occupation_key, specialty_key, &seg. from merged_users;
quit;
