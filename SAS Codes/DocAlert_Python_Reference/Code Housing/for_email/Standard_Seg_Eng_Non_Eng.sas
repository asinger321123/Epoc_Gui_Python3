...
WHERE a.docalert_schedule_key -937 in (586069,
586065,
586067,
589844,
589842,
...
select userid, occupation_key, specialty_key, user_type, &seg.
...
select distinct	t.&seg., t.user_type,
	t.userid, 
	case when (f1.label = 'MD' OR f1.label = 'DO') then CATX(' - ','MD/DO',f2.label) else f1.label end as OCCSPEC
from lmt._matched_%trim(&ncount.) t 
left join edwf.format_lookup (where=(fmtname='OK2ON')) f1 on t.occupation_key = f1.key_value
...
%local i pricing;
%let pricing = user_type;

proc sql;
create table target_total_dedup_2 as
select distinct userid, occupation_key, specialty_key, user_type, &seg. from merged_users;
quit;

proc freq data=target_total_dedup_2 noprint;
tables &pricing. / out=&pricing._target nopercent norow nocol;
run;
