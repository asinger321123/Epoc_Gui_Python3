...
%global lm_bda_price;
	...
	%if &list_match_type. = None and &matched_file. = "" %then %do;
		...
		proc sql;
		create table bda_count_price as
		select 'Behavioral Segment**' as type, BDA_count, round((BDA_price+499),1000) as BDA_price
		FROM bda_results;
		quit;
		...
		proc sql;
		create table lm_plus_bda1 as
		select BDA_price format=13. FROM bda_count_price;
		run;
		...
		proc sql noprint;
		select BDA_price format=dollar10. into: lm_bda_price from lm_plus_bda;
		quit;
		%put &lm_bda_price;
		...
		proc sql noprint;
		select count1 format=comma8. into: lm_bda_count from lm_bda_users;
		quit;
		%put &lm_bda_count;
